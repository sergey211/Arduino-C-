    ...
    if (client.available()) {
      ...
      if (readString.length() < 30) {
        ...
        //Level=0 - выключен
          ...
          if (LEDON){
            client.print("<form method=get name=LED><input type=radio name=Level value=1 CHECKED>Включить");
client.print("<input type=radio name=Level value=0>Выключить");
client.println("<input type=submit value=Применить></form>");
            client.println("<font size=’5′>LED-статус: ");
            client.println("<font size=’5′>Вкл.");
          }else{
            client.print("<form method=get name=LED><input type=radio name=Level value=1>Включить");
            client.print("<input type=radio name=Level value=0 CHECKED>Выключить");
            client.println("<input type=submit value=Применить></form>");
            client.println("<font size=’5′>LED-статус: ");
